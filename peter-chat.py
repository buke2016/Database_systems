...
API_KEY = os.getenv('C:\\Users\\bukep\\Desktop\\Database_systems\\.env')
...
if response.status_code == 200:
    ...
    # Insert data into the table, filtering for relevant fields (replace with actual API fields)
    for row in data['response']['data']:
        ...
        # Extract energy type (replace with actual key from API data)
        energy_type = row.get('fueltypeid', 'UNKNOWN')

        # Extract consumption and production (replace with actual keys from API data)
        consumption = row.get('value', 0)  # Assuming 'value' represents consumption or production

        # Here we assume that 'value' field stores either consumption or production, but you may need to modify this
        # based on how the API differentiates between the two.
        production = consumption  # This is just an example, adjust based on actual API response
    ...
    try:
        # Assume 'EXPORT' data is indicated by energy_type 'EXPORT'
